...
| 2b | Pending (auto-broadcast client), order wait time counting | Order broadcasting soon | "{N} minutes to broadcast." (live, updates each minute, drawer-only) — **the same countdown, from the same base, drives the Dispatch Logs tab's On Hold state (§7.5.2)** | View Activity | Dynamic |
...
|---|---|---|
| 3.3 | Aug 2026 | **One hold-window countdown, not two** (ruled 2026-08-04, corrections pass): the Overview tab's row-2b "{N} minutes to broadcast." (§7.2) and the Dispatch Logs tab's On Hold state (§7.5.2) had drifted into two independent implementations — a random per-order value in one, a hardcoded 10-second countdown restarting on every tab mount in the other — so switching tabs mid-count showed two different, inconsistent numbers. Clarified: both are **the same `dispatch.orderWaitTime` hold window**, must derive from the same base value, and must be computed against a single elapsed-since-drawer-open clock (not a per-tab-mount timer) so they always agree regardless of which tab is open or how long the drawer has been open. §7.2 row 2b and §7.5.2's On Hold row, and the Appendix A flag description, updated to state this explicitly. |
